...
## Release status: incomplete, setup preview only

The accepted milestone is **Complete Keep Up the Fire**, followed by standalone Normandy Mission 1, then campaign continuity. This change does not close that milestone. The public mission catalog and `createMission` reject starting Keep Up the Fire while its readiness requirements remain unmet. Normandy 1 is also unavailable. Do not remove these gates merely because the regression suite passes.

Company Assault remains the playable regression course. Its scenario content version remains 4. The extended engine/replay contract is rules revision 9; historical revision-8 exports remain unchanged and cannot be strictly loaded as revision 9. Keep Up the Fire draft content is version 9; version-1/2/3/4/5/6/7/8 development replays are incompatible. The existing weighted combat distributions remain unchanged.
