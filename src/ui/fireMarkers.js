...
export function fireMarkers(view,location) {
  const allFire=view.fire.filter(f=>f.target===location.id),fire=allFire.filter(f=>!f.pdf_only),markers=[];
...
export function fireExplanation(view,fire) {
  ...
  if(fire.pdf_only)return `${source?.name??'Unidentified mortar'}: temporary direct-lay PDF toward ${view.locations.find(l=>l.id===fire.target)?.name??fire.target}. It counts for crossfire even after a miss, adds no basic VOF, and is removed at cleanup.`;
  ...
  const reason=fire.reason==='GRAZING_FIRE'?'Tripod grazing fire affects this card along the same firing direction; it is not an additional weapon.':fire.reason==='CONTINUING_AT_CLEARED_POSITION'?'Established fire held at a cleared position.':fire.reason==='INTERCEPTED_OR_FOLLOWING'?'Movement changed the affected card along the established direction.':fire.reason?.startsWith('BLOCKED')?'Smoke or Incoming fire interrupts the outward path.':'Established engagement.';
