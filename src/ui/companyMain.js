...
import {fireMarkers,pdfDirections,pdfPaths} from './fireMarkers.js';
...
const presentation=()=>({selected,action,target,generalIssuer,contributors,selectedPdf,combatId,combatStage,combatOpen,contactAcknowledged,feedback,feedbackKind});
...
function render(){
  ...
  app.innerHTML=`${commandHeader({v,name,seed,recovery,recoveryPending,locked,combatBlocked,advanceLabel,phaseIndex,progress,feedback,feedbackKind})}
    ...
    <div class="map-layer"><svg id="fire-overlay" aria-hidden="true"></svg><div class="company-map">${[...v.locations].sort((a,b)=>b.row-a.row||a.col-b.col).map(l=>{
      ...
      return `<article data-location="${l.id}" class="terrain-card ${!selected||l.id===unit.location||unit.los.includes(l.id)?'in-los':'outside-los'} ${combat?.target_location===l.id||contactLocations.has(l.id)?'combat-focus':''} ${l.staging?'staging':''} ${fire.length||support.length?'incoming':''}">${terrainBorders(l)}<div class="terrain-header"><h3>${esc(l.name)}</h3>${l.staging?'':`<span class="protection">+${l.protection}</span>`}</div>
  ...
  app.querySelectorAll('[data-pdf-path]').forEach(button=>button.onclick=()=>{selectedPdf=button.dataset.pdfPath===selectedPdf?null:button.dataset.pdfPath;drawFirePaths(v);persist();});
