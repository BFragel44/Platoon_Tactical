import { borders, DIRECTIONS, TERRAIN_SOURCES } from '../sim/company/terrain.js';
// CAC final run: Basic Training pp. 34, 48, 50. Coordinates are terrain cards.
const terrain = {
  open: { name: 'Open Fields', protection: 0, cover_draw: 2, cover_limit: 1, borders: borders(DIRECTIONS), burst: 0 },
  marsh: { name: 'Marsh', protection: 1, cover_draw: 2, cover_limit: 1, borders: borders(), burst: 1 },
  woods: { name: 'Woods', protection: 2, cover_draw: 4, cover_limit: 3, borders: borders(), burst: -1 },
  orchard: { name: 'Orchard', protection: 1, cover_draw: 3, cover_limit: 2, borders: borders(), burst: -1 },
  gullyEW: { name: 'Gully · east/west', protection: 2, open_protection: 1, cover_draw: 3, cover_limit: 1, borders: borders(['E','W']), burst: 0 },
  gullyNS: { name: 'Gully · north/south', protection: 2, open_protection: 1, cover_draw: 3, cover_limit: 1, borders: borders(['N','S']), burst: 0 },
};
...
for (let row = 0; row <= 3; row++) for (let col = 1; col <= 4; col++) {
  ...
  locations.push({ ...terrain[type], id: `r${row}c${col}`, row, col, terrain: type,
    ...
    elevation: hill ? 2 : 1, borders: row === 0 ? null : hill ? borders() : {...terrain[type].borders}, terrain_source: row === 0 ? null : TERRAIN_SOURCES[type], hill_source: hill ? TERRAIN_SOURCES.hill : null, staging: row === 0, ...(row===0?{cover_limit:0,cover_draw:0}: {}) });
}
const units = [];
...
export const companyAssault = {
  id: 'company_assault', ruleset: 'company-v1', version: 4, turn_limit: 10, locations, units,
